...
def test_internal_execution_dispatch_rejects_missing_trigger_for_stop_market_order() -> None:
    app = create_app(settings=_settings(), state=build_default_state(default_mode="MOCK"))
    client = TestClient(app)
    payload = _bridge_payload()
    ...
    response = client.post("/internal/execution/dispatch", json=payload)

    assert response.status_code == 422
    assert "trigger_price must be positive" in response.json()["detail"]


def test_internal_execution_dispatch_status_operation_requires_order_identifier() -> None:
    app = create_app(settings=_settings(), state=build_default_state(default_mode="MOCK"))
    client = TestClient(app)
...
def test_internal_execution_dispatch_status_operation_returns_order_state() -> None:
    app = create_app(settings=_settings(), state=build_default_state(default_mode="MOCK"))
    client = TestClient(app)

    response = client.post("/internal/execution/dispatch", json=_status_payload())
